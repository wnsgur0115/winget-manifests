...
:: ----------------------------------------------------------------------------------
:: CMD 창의 코드페이지를 UTF-8로 강제 변경 (한글 깨짐 방어)
chcp 65001 > nul
title Winget Auto Manager (MARM Ultimate Automation)
echo.
...
# ---------------------------------------------------------
# [Step 0] 사전 의존성 검사 (GitHub CLI)
# ---------------------------------------------------------
Clear-Host
Print-Msg "=== Winget 저장소 관리 도구 (Ver. MARM Ultimate Automation) ===" "Green"

if (-not (Get-Command "gh" -ErrorAction SilentlyContinue)) {
    Write-Host "`n------------------------------------------------" -ForegroundColor Red
    Print-Msg "[Missing Dependency] GitHub CLI (gh)가 설치되지 않았습니다." "Red"
    Print-Msg "Release 자동 업로드를 위해 GitHub CLI가 필요합니다." "Yellow"
    Write-Host "------------------------------------------------" 
    
    Write-Host -NoNewline ">> 지금 설치하시겠습니까? (Winget 사용) [Y/n] : "
    $install = Read-Host
    if ($install -eq 'n') {
        Print-Msg "설치를 거부하여 스크립트를 종료합니다." "Red"
        exit
    }
    
    Print-Msg "GitHub CLI 설치 중... (설치 후 스크립트를 재실행해야 할 수 있습니다)" "Cyan"
    winget install GitHub.cli
    
    Print-Msg "설치 완료. 환경 변수 적용을 위해 스크립트를 닫고 다시 실행해주세요." "Green"
    Pause
    exit
} else {
    Print-Msg "[System] GitHub CLI (gh) 감지됨. Release 자동화 기능 활성화." "Gray"
}
...
if ([string]::IsNullOrWhiteSpace($TargetFolder)) {
    Write-Host "`n------------------------------------------------"
    Print-Msg "[Skip] 폴더 이름 미입력 -> Git 동기화 모드로 직행합니다." "Green"
    Write-Host "------------------------------------------------`n"
    $SkipWinget = $true
} else {
...
# ---------------------------------------------------------
# [Step 3] WingetCreate & GitHub Release 자동화
# ---------------------------------------------------------
if (-not $SkipWinget) {
    
    # [3-1] 로컬 바이너리 파일 감지 및 업로드
    $Binaries = Get-ChildItem -Path . -Include *.zip,*.exe,*.msi -File
    
    if ($Binaries.Count -gt 0) {
        $BinFile = $Binaries[0] # 첫 번째 파일 선택
        ...
        if ($doUpload -ne 'n') {
            # 태그 입력
            $ReleaseTag = Read-Host ">> 릴리즈 태그(버전)를 입력하세요 (예: 1.0.0, v1.2) "
            if ([string]::IsNullOrWhiteSpace($ReleaseTag)) { $ReleaseTag = "v$((Get-Date).ToString('yyyyMMdd'))" }

            Print-Msg "GitHub Release 생성 및 업로드 중... (gh release create)" "Cyan"
            try {
                # gh 명령어로 릴리즈 생성 및 파일 업로드
                gh release create $ReleaseTag $BinFile.Name --title "$ReleaseTag" --notes "Uploaded via MARM Auto Manager"
                
                if ($LASTEXITCODE -eq 0) {
                    # URL 추출 로직
                    $RepoUrl = git remote get-url origin
                    $RepoUrl = $RepoUrl -replace '\.git$', ''  # .git 제거
                    $DownloadUrl = "$RepoUrl/releases/download/$ReleaseTag/$($BinFile.Name)"
                    
                    Print-Msg "업로드 성공!" "Green"
                    Print-Msg "생성된 URL: $DownloadUrl" "Green"
                    
                    # 클립보드 복사
                    Set-Clipboard -Value $DownloadUrl
                    Print-Msg ">> [편의 기능] URL이 클립보드에 복사되었습니다! (Ctrl+V로 붙여넣으세요)" "Yellow"

                    # 로컬 파일 정리 질문
                    Write-Host -NoNewline "`n>> 로컬 파일($($BinFile.Name))을 삭제하여 용량을 확보하시겠습니까? [Y/n] : "
                    $delFile = Read-Host
                    if ($delFile -ne 'n') {
                        Remove-Item $BinFile.FullName
                        Print-Msg "로컬 파일이 삭제되었습니다." "Gray"
                    }
                } else {
                    Print-Msg "GitHub CLI 업로드 중 오류가 발생했습니다." "Red"
                }
            } catch {
                Print-Msg "GitHub CLI 실행 실패: $_" "Red"
            }
        }
    }

    # [3-2] WingetCreate 실행 (스마트 모드)
    Write-Host "------------------------------------------------`n"
    $HasYaml = (Get-ChildItem -Path . -Filter "*.yaml").Count -gt 0

    if ($HasYaml -and -not $IsNewFolder) {
        Print-Msg "[Smart Mode] 기존 YAML 파일 감지 -> 'update' 모드 추천" "Green"
        $WingetCmd = "update"
    } else {
        Print-Msg "[Smart Mode] 파일 없음/새 폴더 -> 'new' 모드 추천" "Green"
        $WingetCmd = "new"
    }

    Print-Msg "명령어 실행: wingetcreate $WingetCmd" "Cyan"
    if ($Binaries.Count -gt 0 -and $doUpload -ne 'n') {
        Print-Msg "팁: 'InstallerUrl' 입력 시 Ctrl+V를 누르면 방금 업로드한 주소가 입력됩니다." "Yellow"
    }

    try {
        Invoke-Expression "wingetcreate $WingetCmd"
    } catch {
        Print-Msg "[Exec Error] wingetcreate 실행 중 오류 발생." "Red"
        Print-Msg "상세 에러: $_" "Red"
        Pause
        exit
    }
    ...
    Write-Host "`n"
    Print-Msg "Manifest 작업 완료. Git 작업을 준비합니다." "Green"
    Confirm-Continue
}
...
try {
    ...
    Print-Msg "롤백 완료. 작업 내용은 보존되었습니다." "Green"
    Write-Host "------------------------------------------------`n"
    exit
}
